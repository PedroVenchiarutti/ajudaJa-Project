...
const editProfile = () => {
    ...
    const uploadImage = () => {
      ...
      useEffect(()=>{
        Api.get(`/private/client/${id}`, config).then(resp => { 
            setClient({
      ...
      const {username, email, emergencynumber, avatar, helth_insurance, birthday, gender, name, lastname} = client
      ...
      const handleSubmit = url => { 
        ...
        Api.put(`/private/update/users/${id}`, { 
            username,
            email 
        }, config).then(resp => { }).catch(err=> console.log(err))

        Api.put(`/private/client/update/${id}`, { 
            birthday,
            emergencynumber,
            helth_insurance,
            gender, 
                  ...
                  <Fade bottom>
                      ...
                      <div className=" bg-[#fff] drop-shadow-lg  grid-cols-2 m-auto lg:px-16 py-5 rounded-lg pb-10 px-4">
                          <div className="flex justify-center items-center gap-5 ">
                             {preview ? <img className="w-32 h-32 rounded-[50%] border border-[#E3E6E3]" src={preview} alt="" /> : <img className="w-32 h-32 rounded-[50%] border border-[#E3E6E3]" src={client.avatar} alt="" />}
                              
                                  <button className="px-5 py-2 rounded-lg text-white font-bold border  bg-navBg hover:bg-opacity-0 text-sm hover:text-navFontColor hover:border " onClick={()=> 
                                    fileInputRef.current.click()}>Alterar foto</button>
                        ...
                        <input type="file" ref={fileInputRef} accept='image/*' id="fileImg" onChange={(e) => setImageUpload(e.target.files[0])} className="ml-36 absolute hidden placeholder-colorFontParagraph max-w-3xl w-80 mt-[362px] border-b bg-faqGrayBg p-1 "/>
                          ...
                          <div className="pt-4 formAndButtons flex flex-col  content-center ">
                              <form className="flex flex-col gap-5 items-center ">
                               ...
                               <Input info={client.email} label="Email" handleChange={handleChange('email')} />
                               <Input info={emergencynumber} label="Telefone de emergência" 
                               handleChange={handleChange('emergencynumber')} />
                               <Input info={client.helth_insurance} label="Convênio Médico"
                              ...
                              <div className="buttons py-6 pt-14 flex flex-col gap-2 justify-center">
                                  ...
                                  <button className="border px-8 py-2 rounded-lg w-[100%] text-navFontColor font-bold  hover:bg-navBg hover:text-white" onClick={uploadImage}> Salvar</button>
                                  <button className="px-6 py-2 rounded-lg text-white font-bold  bg-navBg hover:bg-opacity-0 border hover:text-navFontColor hover:border " > <Link to="/myprofile">Voltar</Link></button>
